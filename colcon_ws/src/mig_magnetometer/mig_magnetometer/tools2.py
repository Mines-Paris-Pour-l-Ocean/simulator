...
def xyz_to_wgs84(x, y, z, lat0, lon0) :
    ...
    lon, lat = pyproj.transform(proj_utm, proj_wgs84, x, y)
    return lon, lat, z


def wgs84_to_xyz(lat0, lon0) :
    utm_zone = math.floor((lon0 + 180)/6) + 1
    proj_wgs84 = pyproj.Proj(proj = 'latlong', datum='WGS84')
    proj_utm = pyproj.Proj(proj = 'utm', zone = utm_zone, datum='WGS84')
    X_0, Y_0 = pyproj.transform(proj_wgs84, proj_utm, lon0, lat0)
    return X_0, Y_0
...
def magfield_NED(qx,qy,qz,qw,field):
    ...
    return B_NED* 10**(-9) #conversion en mGauss